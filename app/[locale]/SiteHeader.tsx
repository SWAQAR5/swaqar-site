...
// Shared across every page (home, /model, /arms — Stage 3 split) so the banner, nav, and
// language switcher behave identically everywhere, instead of being re-implemented per page.
//
// Link strategy: every in-page-anchor nav/footer link now points at an absolute
// `/{locale}#anchor` URL instead of a bare `#anchor`. When already on the home page this
// resolves to the exact same URL as today (browser just scrolls, no reload — html has
// scroll-behavior:smooth). From any other page (e.g. /model), the same href is a real
// navigation to the home page that lands on the right section once loaded — which is exactly
// what Stage 3 item 2.2 asks for, with no extra client-side routing logic needed.
export default function SiteHeader({ locale }: { locale: string }) {
  ...
  // Mobile menu open/closed as React state rather than the imperative getElementById +
  // classList.toggle the original single-page site used. Found during Stage 3 mobile-toggle
  // testing: an App Router navigation to a different `[locale]` value (e.g. tapping a language
  // button while on /en, landing on /ar) tears down and recreates this component's DOM nodes,
  // so a plain DOM-manipulation burger toggle bound once on mount silently stopped working
  // after the first locale switch — the exact path a mobile user takes to switch language twice
  // in a row. React state re-renders correctly on every mount, remount, or prop change, so it
  // isn't sensitive to how the router happens to handle the DOM underneath it.
    ...
    <>
      ...
      <nav id="nav">
        ...
        <ul className={`nav-links${menuOpen ? ' open' : ''}`} id="navLinks" onClick={() => setMenuOpen(false)}>
          {/* Mobile-only language row — desktop hides this via CSS (see .lang-toggle-mobile in
              swaqar.css) and shows the header .lang-toggle instead. Fixes the Stage 3 mobile
              bug: the header toggle used to collapse to hiding 3 of the 4 language buttons on
              phone widths, leaving only the active one both visible and reachable. */}
          <li className="lang-toggle-mobile-item">
            <div className="lang-toggle-mobile" aria-label={tx(t.a11y.langSelection, lang)}>
              <LangButtons />
            </div>
          </li>
          <li><a href={`/${locale}#mission`}>{tx(t.nav.mission, lang)}</a></li>
          <li><a href={`/${locale}#identity`}>{tx(t.nav.identity, lang)}</a></li>
          <li><a href={`/${locale}#corridors`}>{tx(t.nav.corridors, lang)}</a></li>
          <li><a href={`/${locale}/model`}>{tx(t.nav.model, lang)}</a></li>
          <li><a href={`/${locale}/arms`}>{tx(t.nav.arms, lang)}</a></li>
          <li><a href={`/${locale}#governance`}>{tx(t.nav.governance, lang)}</a></li>
          <li><a href={`/${locale}#contact`} className="nav-cta">{tx(t.nav.engage, lang)}</a></li>
        </ul>
        ...
        <button
          ...
          className="burger"
          id="burger"
          aria-label={tx(t.a11y.openMenu, lang)}
